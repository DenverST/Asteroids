...
class AsteroidsViewController: UIViewController
{
    ...
    private func initializeIfNeeded() {
        if asteroidField == nil {
            asteroidField = AsteroidFieldView(frame: CGRect(center: self.view.bounds.mid, size: view.bounds.size * Constants.asteroidFieldMagnitude))
    ...
    @IBAction func burn(_ sender: UILongPressGestureRecognizer) {
        switch sender.state {
        case .began, .changed:
            ship.direction = (sender.location(in: self.view) - ship.center).angle
            burn()
        case .ended:
            endBurn()
        default:
